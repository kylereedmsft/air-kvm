...
bool HidController::SendKeyType(const String& text) {
  ...
  // Expand the text into the same token stream the native tests exercise:
  //   \n -> Enter, \t -> Tab, \\ -> literal backslash
  //   {Enter}, {Tab}, {Escape}, {Up}, ... -> named special keys
  // Unknown escapes or brace names are sent literally.
  const auto keys = ExpandKeyTypeText(std::string(text.c_str()));
  for (const auto& key_name : keys) {
    uint8_t modifier = 0;
    uint8_t keycode = 0;
    const String key(key_name.c_str());
