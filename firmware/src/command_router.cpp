...
bool CommandRouter::HandleHidCommand(const airkvm::Command& cmd) {
  switch (cmd.type) {
    ...
    case airkvm::CommandType::MouseMoveAbs: {
      const bool ok = hid_.SendMouseMoveAbs(cmd.x, cmd.y);
      if (!ok) transport_.EmitLog("hid.reject mouse.move_abs");
      return ok;
    }
    case airkvm::CommandType::MouseScroll: {
      const bool ok = hid_.SendMouseScroll(cmd.dy);
      if (!ok) transport_.EmitLog("hid.reject mouse.scroll");
      return ok;
    }
